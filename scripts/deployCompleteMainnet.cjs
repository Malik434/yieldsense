...
const AERO_ADDRESS = process.env.AERO_ADDRESS || "0x940181a94A35A4569E4529A3CDfB74e38FD98631";
...
const POOL_ADDRESS = process.env.POOL_ADDRESS || "0x6cDcb1C4A4D1C3C6d054b27AC5B77e89eAFb971d";
const GAUGE_ADDRESS = process.env.GAUGE_ADDRESS || "0x4F09bAb2f0E15e2A078A227FE1537665F55b8360";
...
async function registerIfSet(registry, roleName, envName) {
  ...
  const deploymentHash = process.env[`${envName}_DEPLOYMENT_HASH`] || hre.ethers.ZeroHash;
  const codeHash = process.env[`${envName}_CODE_HASH`] || hre.ethers.ZeroHash;
  await (await registry.registerProcessor(processor, role, deploymentHash, codeHash, { gasLimit: 180_000 })).wait();
...
async function main() {
  ...
  if (balance < hre.ethers.parseEther(process.env.MIN_DEPLOYER_ETH || "0.01")) {
  ...
  await (await autocompounder.setKeeper(keeperAddress, { gasLimit: 120_000 })).wait();
  ...
  const gridVault = await GridVault.deploy(deployer.address, { gasLimit: 2_500_000 });
  ...
  const gridStrategyManager = await GridStrategyManager.deploy(deployer.address, gridVaultAddress, { gasLimit: 4_000_000 });
  ...
  const aeroUsdcPairId = hre.ethers.keccak256(hre.ethers.toUtf8Bytes("AERO-USDC"));
  const ethUsdcPairId = hre.ethers.keccak256(hre.ethers.toUtf8Bytes("ETH-USDC"));
  const acuAddress = process.env.ACU_ADDRESS?.trim();
  const acuUsdcPairId = hre.ethers.keccak256(hre.ethers.toUtf8Bytes("ACU-USDC"));
  ...
  await (await gridVault.setSupportedToken(USDC_ADDRESS, true, { gasLimit: 100_000 })).wait();
  await (await gridVault.setSupportedToken(AERO_ADDRESS, true, { gasLimit: 100_000 })).wait();
  await (await gridVault.setSupportedToken(WETH_ADDRESS, true, { gasLimit: 100_000 })).wait();
  if (acuAddress) {
    assertAddress("ACU_ADDRESS", acuAddress);
    await (await gridVault.setSupportedToken(acuAddress, true, { gasLimit: 100_000 })).wait();
  }
  await (await gridVault.setManager(gridStrategyManagerAddress, { gasLimit: 100_000 })).wait();
  await (await gridVault.setExecutionRouter(gridExecutionRouterAddress, { gasLimit: 100_000 })).wait();
  console.log("GridVault wired.");

  await (await gridStrategyManager.setExecutionRouter(gridExecutionRouterAddress, { gasLimit: 100_000 })).wait();
  ...
  if (acuAddress) {
    configuredGridPairs.push({ label: "ACU/USDC", pairId: acuUsdcPairId, baseToken: acuAddress });
  }

  for (const pair of configuredGridPairs) {
    await (await gridStrategyManager.configurePair(
      pair.pairId,
      pair.baseToken,
      USDC_ADDRESS,
      true,
    ...
    await (await gridExecutionRouter.setPairAllowed(pair.pairId, true, { gasLimit: 100_000 })).wait();
    console.log(`Grid pair configured: ${pair.label} ${pair.pairId}`);
  }

  await (await gridExecutionRouter.setRouterAllowed(ROUTER_ADDRESS, true, { gasLimit: 100_000 })).wait();
  ...
  if (ownerAddress.toLowerCase() !== deployer.address.toLowerCase()) {
    await (await registry.transferOwnership(ownerAddress, { gasLimit: 100_000 })).wait();
    ...
    await (await gridVault.transferOwnership(ownerAddress, { gasLimit: 100_000 })).wait();
    await (await gridStrategyManager.transferOwnership(ownerAddress, { gasLimit: 100_000 })).wait();
    await (await gridExecutionRouter.transferOwnership(ownerAddress, { gasLimit: 100_000 })).wait();
  ...
  console.log(`NEXT_PUBLIC_MAINNET_EXECUTOR_REGISTRY_ADDRESS=${registryAddress}`);
  console.log(`NEXT_PUBLIC_MAINNET_AUTOCOMPOUNDER_ADDRESS=${autocompounderAddress}`);
  console.log(`NEXT_PUBLIC_MAINNET_GRID_VAULT_ADDRESS=${gridVaultAddress}`);
  console.log(`NEXT_PUBLIC_MAINNET_GRID_STRATEGY_MANAGER_ADDRESS=${gridStrategyManagerAddress}`);
  console.log(`NEXT_PUBLIC_MAINNET_GRID_EXECUTION_ROUTER_ADDRESS=${gridExecutionRouterAddress}`);
  console.log(`NEXT_PUBLIC_MAINNET_ASSET_ADDRESS=${USDC_ADDRESS}`);
  console.log(`NEXT_PUBLIC_ETH_USDC_GRID_POOL_ADDRESS=${GRID_ETH_USDC_PRICE_POOL_ADDRESS}`);
  if (acuAddress) {
    console.log(`NEXT_PUBLIC_ACU_TOKEN_ADDRESS=${acuAddress}`);
    console.log(`NEXT_PUBLIC_ACU_USDC_POOL_ADDRESS=${process.env.ACU_USDC_POOL_ADDRESS || ""}`);
  }
  ...
  console.log(`RPC_URL=${process.env.RPC_URL || process.env.BASE_MAINNET_RPC || "https://mainnet.base.org"}`);
  console.log(`DATA_RPC_URL=${process.env.DATA_RPC_URL || process.env.BASE_MAINNET_RPC || "https://mainnet.base.org"}`);
  ...
  console.log(`npx hardhat verify --network baseMainnet ${gridVaultAddress} ${ownerAddress}`);
  console.log(`npx hardhat verify --network baseMainnet ${gridStrategyManagerAddress} ${ownerAddress} ${gridVaultAddress}`);
  console.log(`npx hardhat verify --network baseMainnet ${gridExecutionRouterAddress} ${ownerAddress} ${registryAddress} ${gridStrategyManagerAddress} ${gridVaultAddress}`);
